...
\begin{abstract} %%% DO NOT CHANGE!
	...
	\noindent{Social} behavior in humans are a complex subject that can seem almost impossible to fully understand. A good example of this are usage of sarcasm in interactions, which is famously easily to create misunderstandings, especially when written, because of intonation and voice features are often used to clear markers for a sarcastic comment. Combined with a internet where endless of conversations take place everyday about the most obscure topics makes this a complicated and nuanced classification problem. This creates the interesting question: How well can the the machines, which we run these websites on, understand what is really being said? Therefore this project aims to explore how well different machine learning models can classify sarcasm from sentences made on social media.
	%%% - E1 - %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\noindent
In this section a brief summary of the models used in this report can be found. To every model, there is a short discussion about it's advantages and weak point and an argument why the model was used.
\begin{table*}
	\caption{{\bf Overview of ML models used in this project.} Table where the different models used in the project is presented, their features explained and arguments for their fit to the project.
	}
	...
	\begin{tabular}{| l | l | l | l |}
		...
		\hspace{0.1cm}
		\parbox{5.5cm}{\begin{flushleft}
				               An already pre-trained language model that can be used to classify sarcasm by fine tuning it by setting a linear layer after it and finetuning the whole model on the specific data set. The model is quite large and therefore it can be computationaly intensive to train.
			               \end{flushleft}}
		\hspace{0.1cm}
		               &
		\hspace{0.1cm}
...
\subsection{Dataset}
The dataset that has been used in this project is the following \cite{SARC}. It's a dataset that was retrieved from the website Kaggle.com, and has been created by scraping the the social platform Reddit.com for comments. On the website you can put different labels on your comments to signal what the comment is about. One of these labels stands for that the comment has some sarcastic remarks in it, and this is what is used for classifying the dataset.The total number of comments is exactly 1 million and the dataset is perfectly balanced so 500K is labeled as normal and the other half is labeled as sarcastic. The dataset also contained the text that the comment answered to, what the authors name was, the name of the specific chat forum that the comment was written in and much more but the only data that was used in this project was the original comment and it's sarcasm label.
\subsection{FastText}
For both this model and the BiLSTM, the word embedder FastText is used \cite{grave2018learning}. It is a pre trained model that takes a word and transforms it into a 300 dimensional vector, and therefore makes it easier interpretable to a computer. This model was picked for the project for it's ability to handle Out-Of-Vocabulary words. If it stumbles upon a word that it doesn't recognize it tries to break down the word in smaller words that could make the word make sense. This is great for handling slang and spelling error that might not be included in the FastText set of known words. This model only handles word so special characters are removed before converting the word to a scentence. For sarcasm in text, these provide valuble markers so to not loose this information, 4 sarcasm pointer where extracted from each sentence. The pointers where the following:
\begin{itemize}
	\item Word where all letters where capitalized
	\item The sequence "!!!"
	\item The sequence "?!" or "!?"
	\item The sequnece "..."
\end{itemize}
Other papers like \cite{9420094} and \cite{KumarEtAl2021} use more sarcasm pointers with higher complexity than what's used in this project, but with regards to time scope of the project it was edecided that these four pointers where enough.
\subsection{Logistic Regression}
\noindent
This model is the simplest, and should have a hard time classifying sarcasm. Because of the fact that the comments can be of different lengths and to train a model input dimension must be constant, it was decided to take all the words in the comments and take the mean of this vector to create a sentence vector for every datapoint. This meant that the model only can learn roughly what the comment was about and all relationships between words get's lost. This vector is then extended with the sarcasm pointers and is then feed through the linear layer to a single value which get's classified with a sigmoid function and a threshold. A full flow chart of the model can be found in figure \ref{fig:selectedmethod}.
\subsection{BiLSTM with multi-head attention}
\noindent
This model is highly inspired by \cite{KumarEtAl2021} with the difference that less sarcasm pointers where used. As in the Logistic Regression we first use the FastText to get the words into vectors, but instead of creating a mean sentence vector, we create a word matrix where every word is a column vector. This is then feed to the BiLSTM where the "Bi" means that we both run the LSTM from the first to the last word and from the last to the first word, which creates two different hidden states in each time step. This so that words both before and after a specific word should be able to impact the hidden state. Now we have a matri


\begin{figure*}[h!]
	\centering
	\includegraphics[width=\textwidth]{figures/Flowchart Whiteboard in Bright Green Lime Green Pink Corporate Neon Style.pdf}
	\caption{{\bf Flowchart of the models used in the project:}
		Architectural comparison of three sarcasm detection models. Top: a Logistic Regression baseline combining FastText sentence vectors and sarcasm pointers. Middle: a BiLSTM with multihead attention, using FastText word embeddings and sarcasm pointers as input. Bottom: a BERTweet transformer model with tokenization and a linear layer. All models output a prediction via sigmoid activation and thresholding.
	}
...
\begin{figure}[!h]
	\centering
	\includegraphics[width=\columnwidth]{figures/logreg_confusion_matrix.pdf}
...
\begin{figure}[!h]
	\centering
	\includegraphics[width=\columnwidth]{figures/LSTM_confusion_matrix.pdf}
	\caption{{\bf Title Figure.} Write some explanatory text for the figure. Make sure that the figure is referenced in the text. The caption should be such that, together with the figure, allows the reader to understand the concepts in the figure itself.}
	\label{fig:res2}
\end{figure}
\begin{figure}[!h]
	\centering
	\includegraphics[width=\columnwidth]{figures/BERTweet_confusion_matrix.pdf}
	\caption{{\bf Title Figure.} Write some explanatory text for the figure. Make sure that the figure is referenced in the text. The caption should be such that, together with the figure, allows the reader to understand the concepts in the figure itself.}
	\label{fig:res2}
\end{figure}
